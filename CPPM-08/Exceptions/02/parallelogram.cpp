...
Parallelogram::Parallelogram(int a, int b, int A, int B) : Quadrangle{ a, b, a, b, A, B, A, B, "Параллелограмм" } { checkParallelogram(); }

void Parallelogram::check() const {
	checkParallelogram();
}

void Parallelogram::checkParallelogram() const {
	if ((a != c || b != d) && (A != C || B != D)) {
